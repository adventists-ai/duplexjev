...
The scripts in `duplexjev/`, `evaluation/`, `training/`, `benchmarks/` and `examples/` are the exact versions used
for the paper. They still reference our cluster layout:

| name in code | meaning |
|---|---|
| `jev_qwen` | this repository's `duplexjev/` package (internal name) |
